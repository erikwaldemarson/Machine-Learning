...
for kfold = 1:K
    ...
    estind = [randomind(1:location), randomind(location+hop+1:N)]; % Select estimation indices
    ...
    for klam = 1:Nlam
        ...
        SEval(kfold,klam) = Nval^(-1)*norm(t(valind)-X(valind,:)*what)^2; % Calculate validation error for this estimate
        SEest(kfold,klam) = (N-Nval)^(-1)*norm(t(estind)-X(estind,:)*what)^2; % Calculate estimation error for this estimate
